...
-- Точная копия функции scanDescendants из вашего бэкдора
local function scanDescendants(parent)
    ...
    for i = 1, #descendance do
        ...
        elseif remoteClass == "RemoteFunction" then
            ...
            -- Точный таймаут 1 секунда как в вашем коде
...
-- Точная копия функции scanGame из вашего бэкдора
local function Scan()
    ...
    if found then
        ...
        AddLog("-- Backdoor scan complete!")
        
        -- Показываем поле ввода и кнопку EXE
